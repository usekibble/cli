...
import { createSource } from "../dist/sources/index.js";
...
for (const name of ["core", "cli"]) {
  ...
  try {
    ...
    results.push({ name: source.name, error: err.message });
  }
}

const m = (n) => `${(n / 1e6).toFixed(1)}M`;
console.log(`window ${since}..${until}  (claude-code only)\n`);
console.log(`  ${"source".padEnd(16)} ${"messages".padStart(9)} ${"tokens".padStart(9)} ${"delta".padStart(9)}`);
console.log(`  ${"transcripts".padEnd(16)} ${String(truth.messages).padStart(9)} ${m(truth.tokens).padStart(9)} ${"-".padStart(9)}`);

let failed = false;
for (const r of results) {
  if (r.error) {
    console.log(`  ${r.name.padEnd(16)} ${"ERROR".padStart(9)}  ${r.error}`);
    continue;
  }
  const delta = truth.tokens ? ((r.tokens - truth.tokens) / truth.tokens) * 100 : 0;
  const flag = Math.abs(delta) <= 2 ? "" : "   <- outside +/-2%";
  console.log(
    `  ${r.name.padEnd(16)} ${String(r.messages).padStart(9)} ${m(r.tokens).padStart(9)} ${`${delta >= 0 ? "+" : ""}${delta.toFixed(1)}%`.padStart(9)}${flag}`,
  );
  if (r.name === "tokscale-core" && Math.abs(delta) > 2) failed = true;
}

console.log(`\n  ${truth.duplicates} duplicate records skipped (same response in more than one transcript file)`);

if (failed) {
  console.error("\nFAILED: the default source is more than 2% off the transcripts.");
  process.exit(1);
}
console.log("\nOK  default source is within 2% of the transcripts");
